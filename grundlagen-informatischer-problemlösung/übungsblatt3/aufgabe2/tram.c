...
{
  if (trip.start.line > MAX || trip.start.stop > MAX || trip.end.line > MAX || trip.end.stop > MAX ||
      trip.start.line < MIN || trip.start.stop < MIN || trip.end.line < MIN || trip.end.stop < MIN)
    return false;
  return true;
}
...
bool is_short_trip(struct trip trip)
{
  if ((trip.start.line == 5 && trip.start.stop == 2) && (trip.end.line == 4 && trip.end.stop == 2) ||
      (trip.start.line == 4 && trip.start.stop == 2) && (trip.end.line == 5 && trip.end.stop == 2))
    return false;

  if ((trip.start.line == 0 && trip.start.stop == 0) && (trip.end.line == 2 && trip.end.stop == 1) ||
      (trip.start.line == 2 && trip.start.stop == 1) && (trip.end.line == 0 && trip.end.stop == 0))
    return false;
